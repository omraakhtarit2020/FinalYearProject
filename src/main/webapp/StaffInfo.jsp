...
<body>
	...
	<%@page
		import="StaffInformation.StaffInfoDao,java.util.*,StaffInformation.MyDetailsData"%>
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

	<%
	StaffInfoDao stf = new StaffInfoDao();
	String dep = request.getParameter("dep");
	List<MyDetailsData> list = stf.getAllInfo(dep);
	...
	<table class="table table-striped">
		...
		<c:forEach items="${list}" var="u">
			<tr>
				...
				<td>${u.getSubject()}</td>
				<td>${u.getHighestQualification()}</td>
				<td>${u.getAnyOther()}</td>
				<td>${u.getTeachingExp()}</td>
				<td>${u.getTeachingMethods()}</td>
				<td>${u.getTechnologyUses()}</td>
				<td>${u.getSmd()}</td>
				<td>${u.getCeqt()}</td>
			</tr>
		</c:forEach>
	...
	<div class="container my-2">
  		<div class="row justify-content-md-center">
    		...
    		<div class="col-md-auto">
      			<button type="button" class="btn btn-success" id="saveAs">Save As</button>
    		</div>
	...
	<script type="text/javascript">
		...
		saveAs.addEventListener('click', saveAsClickHandler);
		...
		function saveAsClickHandler(){
		    ...
		    xhr.onload = function (){
		 	...
		 	var staffInfo = [];
		 	<c:forEach items="${list}" var="u">
				staffInfo.push(`${u.getfName()}${" "}${u.getmName()}${" "}${u.getlName()}${","}${u.getDesignation()}${","}${u.getSubject()}${","}${u.getHighestQualification()}${","}${u.getAnyOther()}${","}${u.getTeachingExp()}${","}${u.getTeachingMethods()}${","}${u.getTechnologyUses()}${","}${u.getSmd()}${","}${u.getCeqt()}`)
